...
const CraftItemsCard = ({single}) => {
    const { image, _id, item_name, subcategory_Name, short_description, Price, rating,processing_time, stockStatus,user_Name } = single;
    return (
        <div>
           <div className="flex flex-col justify-center duration-1000 items-center border p-2 rounded-xl shadow-xl hover:shadow-2xl">
                ...
                <div className="text-center space-y-3  text-[#f9f9f9] font-playfair font-semibold text-xl">
                    ...
                    <div>
                        <Link to={`/details/${_id}`} className="btn bg-[#fd8a71] btn-block hover:bg-[#fe785a] text-white font-bold py-2 px-4 border-b-4 border-orange-700 hover:border-orange-500 rounded join-item">View</Link>
                    </div>
                </div>
            </div>
        </div>
